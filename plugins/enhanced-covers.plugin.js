...
(function() {
  ...
  function cleanupStaleCovers() {
    const continueWatchingRow = document.querySelector('[class*="continue-watching-row"]');
    if (!continueWatchingRow) return;
    
    const posters = continueWatchingRow.querySelectorAll('img[class*="poster-image"][data-enhanced-cover]');
    ...
    posters.forEach((img) => {
      ...
      if (storedImdbId && currentImdbId && storedImdbId !== currentImdbId) {
        ...
        console.log(`[EnhancedCovers] Detected stale cover: was ${storedImdbId}, now ${currentImdbId}`);
        ...
        if (posterContainer) {
          const oldLogo = posterContainer.querySelector('.enhanced-logo-overlay');
    ...
    const allLogos = continueWatchingRow.querySelectorAll('.enhanced-logo-overlay');
  ...
  function replaceCover() {
    ...
    const posters = continueWatchingRow.querySelectorAll('img[class*="poster-image"]');
    ...
    posters.forEach((img) => {
      ...
      if (imdbId) {
        ...
        if (posterContainer) {
          const existingLogo = posterContainer.querySelector('.enhanced-logo-overlay');
        ...
        for (const mutation of mutations) {
          ...
          if (mutation.removedNodes.length > 0) {
            for (const node of mutation.removedNodes) {
              if (node.nodeType === 1) {
                if (node.matches?.('[class*="meta-item"]') ||
          ...
          if (mutation.addedNodes.length > 0) {
            for (const node of mutation.addedNodes) {
              if (node.nodeType === 1) {
                if (node.matches?.('[class*="continue-watching"]') ||
