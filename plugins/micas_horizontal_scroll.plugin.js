 ...
 * @description Horizontal scroll with drag and wheel
 * @version 1.0.3 
 * @author Micas
 */
...
(() => {
    ...
    function enableGrid(grid) {
        ...
        const animate = () => {
            ...
            if (Math.abs(diff) < 0.5) { grid.scrollLeft = targetScroll; isAnimating = false; return; }
        ...
        // TECLADO: só carrossel
        grid.setAttribute('tabindex', '0');
        grid.addEventListener('keydown', (e) => {
            if (!computeIsCarousel(grid)) return;
            if (e.key === 'ArrowRight') { e.preventDefault(); targetScroll += grid.clientWidth * KEY_SCROLL; startAnimation(); }
            else if (e.key === 'ArrowLeft') { e.preventDefault(); targetScroll -= grid.clientWidth * KEY_SCROLL; startAnimation(); }
        });
    ...
    const scanAndEnable = () => {
        ...
        ).forEach(c => { try { if (isLikelyGrid(c)) enableGrid(c); } catch {} });
    };
